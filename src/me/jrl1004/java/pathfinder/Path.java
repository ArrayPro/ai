...
public class Path {
    ...
    Path(Location startLoc, Location endLoc, int recursion, Path master) {
        // Initiation
        this.master = master;
        direction = null;
        ...
        if (!reachesDestination()) {
            ...
            if (isIntercepted()) {
                ...
                moves = paths.lastKey();
                if (moves == 0) {
                    nextPath = new Path(endLoc, localEnd, recursion + 1, master == null ? this : master);
                    // calculateWorkaroundAsynchronously();
                } else
                    nextPath = new Path(localEnd, endLoc, recursion + 1, master == null ? this : master);
            } else {
                nextPath = new Path(localEnd, endLoc, recursion + 1, master == null ? this : master);
            }
